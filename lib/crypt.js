...
class Crypt {
    ...
    signature(privateKey, message) {
        ...
        var csum = forge.md.sha256.create();
        csum.update(message, 'ascii');
	...
	var pss = forge.pss.create({
        ...
        var signature = privateKey.sign(csum, pss);
	...
	var pss = forge.pss.create({
        ...
        // Verify signature
        var verified = publicKey.verify(csum.digest().getBytes(), signature, pss);
        return verified
    }
    ...
    encrypt(publicKeys, message, signature) {
		...
		mgf1: {
            ...
            var fingerprint = self.fingerprint(publicKey); //Todo: update with fingerprint
